...
describe("POST /api/contacts/:contactId/addresses", () => {
  ...
  it("should be able create address", async () => {
    ...
    expect(response.body.data.city).toBe("Bogor");
    expect(response.body.data.province).toBe("Jawa Barat");
    expect(response.body.data.country).toBe("Indonesia");
    expect(response.body.data.postal_code).toBe("16640");
  });
...
describe("PUT /api/contacts/:contacId/addresses/:addressId", () => {
  beforeEach(async () => {
    await UserTest.create();
    await ContactTest.create();
    await AddressTest.create();
  ...
  it("should reject to update address if address is not found", async () => {
    ...
    const response = await supertest(web)
      .put(`/api/contacts/${contact.id}/addresses/${address.id + 12}`)
      .set("X-API-TOKEN", "test")
      .send({
        street: "jalan belum ada",
        city: "Bogor",
  ...
  it("should reject to update address if contacts is not found", async () => {
    const contact = await ContactTest.get();
    const address = await AddressTest.get();

    const response = await supertest(web)
      .put(`/api/contacts/${contact.id + 12}/addresses/${address.id}`)
      .set("X-API-TOKEN", "test")
      .send({
        street: "jalan belum ada",
        city: "Bogor",
...
describe("DELETE /api/contacts/:contactId/addresses/:addressId", () => {
  ...
  it("should reject to remove address if address is not found", async () => {
    ...
    const response = await supertest(web)
      .delete(`/api/contacts/${contact.id}/addresses/${address.id + 11}`)
      .set("X-API-TOKEN", "test");
...
describe("GET /api/contacts/:contactId/addresses", () => {
  ...
  it("should reject list address if contact is not found", async () => {
    ...
    const response = await supertest(web)
      .get(`/api/contacts/${contact.id + 10}/addresses`)
      .set("X-API-TOKEN", "test");
